...
while True:
    ...
    if len(todas_maos) == 1:
        ...
        if todas_maos[0]['lado'] == 'Left':
            ...
            for indice_linha, linha_teclado in enumerate(teclas):
                for indice, letra in enumerate(linha_teclado):
                    ...
                    if offset+indice*80<indicador_x<100+indice*80 and offset+indice_linha*80<indicador_y<100+indice_linha*80:
                        img = imprime_botoes(img, (offset + indice*80, offset + indice_linha*80), letra, cor_retangulo=GREEN)
                        if indicador_z < -85:
                            contador = 1
                            escreve = letra
                            img = imprime_botoes(img, (offset + indice*80, offset + indice_linha*80), letra, cor_retangulo=AZUL)
            ...
            cv2.rectangle(img, (offset, 450), (830, 500), AZUL, cv2.FILLED, 1)
            cv2.putText(img, texto[-40:], (offset, 480), cv2.FONT_HERSHEY_COMPLEX, 1, PRETO, 2)
            cv2.circle(img, (indicador_x, indicador_y), 7, AZUL_ESCURO, cv2.FILLED)



        if todas_maos[0]['lado'] == 'Right':
            if info_dedos_mao1 == [True, False, False, False] and office_word == False:
            ...
            if info_dedos_mao1 == [False, False, False, False] and office_word == True:
                office_word = False
